...
*}
    ...
    <table class="opartindexnowAdminTable table table-striped">
        ...
        {foreach $categories as $category}
            <tr onClick="opartToggleExcludeCategory({$category.id_category|intval})">
                <td>{$category.id_category|intval}<input type="hidden" name="listItem[]" value="{$category.id_category|intval}" /></td>
                <td>{$category.name|escape:'htmlall':'UTF-8'}</td>
                <td></td>
                <td><input type="checkbox" name="NoIndexNow[{$category.id_category|intval}]" value="{$category.id_category|intval}" id="CategoryIndexNow[{$category.id_category|intval}]" class="nocheckbox" {if $category.exclude == 1}checked="checked"{/if} /></td>
            </tr>
        {/foreach}
