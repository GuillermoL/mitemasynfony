...
*}
  ...
  <table class="opartindexnowAdminTable table table-striped">
        ...
        {foreach $articles as $article}
            <tr onClick="opartToggleExcludeAdvancedSearch({$article.id_seo|intval})">
                <td>{$article.id_seo|intval}<input type="hidden" name="listItem[]" value="{$article.id_seo|intval}" /></td>
                <td for="NoIndexNow[{$article.id_seo|intval}]">{$article.title|escape:'htmlall':'UTF-8'}</td>
                <td></td>
                <td><input type="checkbox" name="NoIndexNow[{$article.id_seo|intval}]" id="AdvancedSearchIndexNow[{$article.id_seo|intval}]" class="nocheckbox" value="{$article.id_seo|intval}" {if $article.exclude == 1}checked="checked"{/if} /></td>
            </tr>
        {/foreach} 
