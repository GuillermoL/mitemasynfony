...
*}
 ...
 <table class="opartindexnowAdminTable table table-striped">
        ...
        {foreach $manufacturers as $manufacturer}
            <tr onClick="opartToggleExcludeManufacturer({$manufacturer.id_manufacturer|intval})">
                <td>{$manufacturer.id_manufacturer|intval}<input type="hidden" name="listItem[]" value="{$manufacturer.id_manufacturer|intval}" /></td>
                <td for="NoIndexNow[{$manufacturer.id_manufacturer|intval}]">{$manufacturer.name|escape:'htmlall':'UTF-8'}</td>
                <td></td>
                <td><input type="checkbox" name="NoIndexNow[{$manufacturer.id_manufacturer|intval}]" id="ManufacturerIndexNow[{$manufacturer.id_manufacturer|intval}]" class="nocheckbox" value="{$manufacturer.id_manufacturer|intval}" {if $manufacturer.exclude == 1}checked="checked"{/if} /></td>
            </tr>
        {/foreach}
